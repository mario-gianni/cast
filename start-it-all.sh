...
# where to run
MY_ROOT="$HOME/Workspace/bleeding-edge"

RUNS_CONFIG_FILE=./RUNS-CONFIG.sh
...
./update-latest-link.sh "$THIS_DIR"
...
print_pwd
gnome-terminal --geometry=80x6 -x tools/abducer/start-abducer-server
sleep 4s
echo "Assuming the abducer is ready now." >&2
...
print_pwd
gnome-terminal -x bash -c 'output/bin/cast-server-start ; sh'
sleep 2s
...
print_pwd
gnome-terminal -x bash -c "output/bin/cast-client-start $CAST_INST_FILE ; sh"
