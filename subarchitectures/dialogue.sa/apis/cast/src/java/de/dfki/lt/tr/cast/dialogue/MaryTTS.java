...
public class MaryTTS extends ManagedComponent {
   ...
   // TTSRemote ttsRemote;
    ...
    public void executeSpeechSynthesisTask(ProcessingData pd,
        ...
        if (soiWM != null) {
            final SpokenOutputItem soi = (SpokenOutputItem) soiWM.getData();
            ...
            try {
                ...
                if (m_bNoMary) {
                    ...
                    //if(soi.phonString.contains("%") || soi.phonString.contains("@") ){
                        ...
                        log("XML file written: ["+l_xmlfile+"]");    // The name of the voice to be used
                       //  static String m_voiceName = "us2";
                        ...
                        m_ttsLocal.speak(soi.phonString, new Runnable() {

							@Override
							public void run() {
								getLogger().info("Mary stopped speaking.");
								if (soi.about != null) {
									getLogger().info("this utterance should change the last mentioned entity -> doing this now");
									publishMentionedEntity(soi.about);
								}
								else {
									getLogger().info("no new topic in the SOI, not doing anything");
								}
							}
...
//         _config.list(System.out);
        ...
        if (_config.containsKey("--no-mary")) {
            ...
            // voice locale
            if (_config.containsKey("--locale")) {
                m_voiceLocale = _config.get("--locale");
            }
	...
	private void publishMentionedEntity(WorkingMemoryPointer entityWMP) {
		long now = System.currentTimeMillis();
		MentionedEntityPointer entityPtr = new MentionedEntityPointer(entityWMP, now);
		try {
			addToWorkingMemory(newDataID(), entityPtr);
		}
