...
import de.dfki.lt.tr.dialogue.interpret.atoms.*;
...
//		nomToRemapper = new HashMap<String, NomToRemapper>();
	...
	protected ProofInterpretationContext<InterpretedUserIntention, String> initContext() {
		...
		solvers.addSolver(new TypeOfQUDAssertionSolver());
                solvers.addSolver(new DecodedCoordsAssertionSolver());
                solvers.addSolver(new IsGroundedForwardSolver());
...
//			}
	...
	public class ReferenceResolutionAssertionSolver extends AbstractAssertionSolver<AssertedReferenceAtom> {
		...
		public ContextUpdate solveFromParsed(AssertedReferenceAtom a) {
			...
			ResultGatherer<ReferenceResolutionResultWrapper> gatherer = new ResultGatherer(wma, new MixingCombinator(getLogger(), 2));
...
//			}
        ...
        public class DecodedCoordsAssertionSolver extends AbstractAssertionSolver<DecodedCoordsAtom> {

		public DecodedCoordsAssertionSolver() {
			super(new DecodedCoordsAtom.Matcher());
		}

		@Override
		public ContextUpdate solveFromParsed(DecodedCoordsAtom a) {
			final dFormula encoded = a.getEncoded();
			if (encoded == null) {
				getLogger().error("encoded string undetermined");
				return null;
			}

                        final String arg = MoveActionFactory.dFormulaToEncodedString(encoded);
                        
                        if (arg != null) {
                            return new ContextUpdate() {
                                @Override
                                public void doUpdate(AbductionEnginePrx engine) {
                                    ModalisedAtom matom = new DecodedCoordsAtom(encoded, arg).toModalisedAtom();
                                    getLogger().debug("will add the following fact: " + PrettyPrint.modalisedAtomToString(matom));
                                    engine.addFact(matom);
                                }
                            };
	...
	public class IsGroundedForwardSolver extends AbstractAssertionSolver<IsGroundedForwardAtom> {

		public IsGroundedForwardSolver() {
			super(new IsGroundedForwardAtom.Matcher());
		}

		@Override
		public ContextUpdate solveFromParsed(IsGroundedForwardAtom a) {
			final String nominal = a.getNominal();
			if (nominal == null) {
				getLogger().error("undetermined nominal");
				return null;
			...
			final WorkingMemoryAddress requestWMA = newWorkingMemoryAddress();

			final Waiter<ThisIsForward> waiter = new Waiter<ThisIsForward>();

			WorkingMemoryChangeFilter filter = ChangeFilterFactory.createAddressFilter(requestWMA, WorkingMemoryOperation.OVERWRITE);
			WorkingMemoryChangeReceiver receiver = new WorkingMemoryChangeReceiver() {
				@Override
				public void workingMemoryChanged(WorkingMemoryChange _wmc) {
					try {
						final ThisIsForward tif = getMemoryEntry(_wmc.address, ThisIsForward.class);
						waiter.set(tif);
					}
					catch (SubarchitectureComponentException ex) {
			...
			ThisIsForward result = null;

			try {
				addChangeFilter(filter, receiver);
				addToWorkingMemory(requestWMA, new WhatIsForward(true));

				result = waiter.get(2, TimeUnit.SECONDS);
			...
			if (result != null) {
				final String tgt = "coordinates=" + Double.toString(result.x) + "," + Double.toString(result.y) + ",0.0";
				getLogger().info("got new coords: " + tgt);
				...
				return new ContextUpdate() {
					...
					public void doUpdate(AbductionEnginePrx engine) {
						...
						IsGroundedForwardAtom igfa = new IsGroundedForwardAtom(nominal, tgt);

						ModalisedAtom matom = igfa.toModalisedAtom();

						getLogger().debug("adding atom to the facts: " + PrettyPrint.modalisedAtomToString(matom));
						engine.addFact(matom);
					}
				...
				getLogger().warn("timed out while waiting for a reply!");
				return null;
			}
		}
...
//		renamedNomToNom.put(newNom, nom);
	...
	public static class ReferenceResolutionResultWrapper extends CASTResultWrapper<ReferenceResolutionResult> {
	...
	public static class MixingCombinator implements ResultCombinator<ReferenceResolutionResultWrapper> {
		...
		public MixingCombinator(Logger logger, int minCount) {
		...
		synchronized public void addResult(ReferenceResolutionResultWrapper added) {
			logger.debug("got a new reference resolution result: " + ReferenceUtils.resolutionResultToString(added.getResult()));
			++count;
	...
	public static class Waiter<T> {
		...
		public T get(int len, TimeUnit unit) {
			try {
				latch.await(len, unit);
				return ref;
			}
