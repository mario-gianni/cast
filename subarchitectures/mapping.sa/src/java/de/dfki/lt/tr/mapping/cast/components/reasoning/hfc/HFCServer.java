package de.dfki.lt.tr.mapping.cast.components.reasoning.hfc;
...
import eu.nifti.mapping._HFCInterfaceDisp;
...
//		}
	 ...
	 * @author zender
	 *
	 */public class HFCInterfaceI extends _HFCInterfaceDisp {
		...
		public QueryResults querySelect(String q, Current current) {
			...
			if (q.toUpperCase().startsWith("SELECT")) {
				debug("received a SELECT type query");
				...
				try {
					...
					if (bt == null) {
					...
					else if (bt.table.size()==0){
						debug("HFC: binding table has size 0");
						_results.bt = new String[0][0];
						_results.varPosMap = new HashMap<String, Integer>();
					}
					else {
						...
						debug("received a decoded collection of size: " + _hfcAnswer.size());
		...
		@Override
		public void addTuple(String[] t, Current current) {
			this.forwardChainer.tupleStore.addTuple(t);
			this.forwardChainer.computeClosure();
			// log("addTuple() needs to be implemented!");
